...
app_startup (GApplication *application) {
  ...
  build = gtk_builder_new_from_resource ("/example6/tfe.ui");
  ...
  /* 我发现get得到的指针不需要解引用，new赋值的指针需要解引用 */
  display = gtk_widget_get_display (GTK_WIDGET (win));
  GtkCssProvider *provider = gtk_css_provider_new ();
  gtk_css_provider_load_from_data (provider, "textview {color: green; padding: 10px; font-family: monospace; font-size: 12pt;}", -1);
...
main (int argc, char **argv) {
  ...
  /* startup 信号是在GtkApplication实例初始化之后发出的 */
  g_signal_connect (app, "startup", G_CALLBACK (app_startup), NULL);
  /* 命令行没有参数输入，触发activate信号 */
